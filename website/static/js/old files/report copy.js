document.addEventListener('DOMContentLoaded', function () {
    ...
    function gatherFilters(type) {
        ...
        if (dateRangeID && $(`#${dateRangeID}`).val()) {
            ...
            const [startDate, endDate] = $(`#${dateRangeID}`).val().split(' - ').map(dateStr =>
                moment(dateStr, 'MM/DD/YYYYY').format('YYYY-MM-DD')
            );
    ...
    let filterStart = moment('1900-01-01');
    ...
    function triggerRecordTableFilter() {
        const selectedType = $('#dataFilterDropdown').val();
        const civilStatus = $('#civilStatusFilter').val();
        const ligitivity = $('#ligitivityFilter').val();
        const startDate = filterStart.format('YYYY-MM-DD');
        const endDate = filterEnd.format('YYYY-MM-DD');

        fetchFilteredTable(selectedType, startDate, endDate, civilStatus, ligitivity);
    }
